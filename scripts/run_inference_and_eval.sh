...
for TRAIN_RUN_ID in "${TRAIN_RUN_IDS[@]}"; do
    ...
    if [[ -z "$INFER_RUN_ID" ]]; then
        ...
        echo "       Check the inference output above for errors." >&2
        exit 1
    fi
...
" "$TEMPLATE_CONFIG" "${PAIRS[@]}" > "$TEMP_CONFIG"

echo ""
echo "==> Generated eval config: $TEMP_CONFIG"
echo "==> Evaluating ${#INFERENCE_RUN_IDS[@]} run(s)..."

if $DRY_RUN; then
    echo "    [dry-run] uv run evaluate --config $TEMP_CONFIG"
    echo ""
