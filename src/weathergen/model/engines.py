...
class EmbeddingEngine(torch.nn.Module):
    ...
    def __init__(self, cf: Config, sources_size) -> None:
        ...
        :param sources_size: List of number of channels for each stream
        """
    ...
    def forward(self, streams_data, pe_embed, dtype, device):
        ...
        :param streams_data: List of streams data.
...
class LocalAssimilationEngine(torch.nn.Module):
    ...
    def __init__(self, cf: Config) -> None:
        ...
        streams (e.g., satellite, station data) within each HEALPix cell. It operates locally, meaning
        attention is computed only among tokens belonging to the same cell. This step aggregates
        high-resolution, heterogeneous input data into a unified cell-level representation before
        global interaction takes place. It uses a sequence of self-attention blocks and MLPs.
        """
...
class GlobalAssimilationEngine(torch.nn.Module):
    ...
    def __init__(self, cf: Config, num_healpix_cells: int) -> None:
        ...
        physical interactions across the entire globe. It alternates between local attention (focusing
        on neighboring cells) and global attention (fully connected or sparse global patterns) to
        efficiently propagate information. This engine transforms the local latents into a globally
...
class EnsPredictionHead(torch.nn.Module):
    ...
    def __init__(
        ...
        :param hidden_factor: Hidden factor.
        :param final_activation: Final activation function.
        """
...
class TargetPredictionEngineClassic(nn.Module):
    ...
    def __init__(
        ...
        latent states back to specific target coordinates (e.g., station locations). It typically employs
        a PerceiverIO-style architecture where target coordinate embeddings query the latent state via
        cross-attention. This engine is "Classic" in the sense that it strictly follows the original
        design with coordinate conditioning and optional self-attention, without the flexible decoder
        types found in the newer `TargetPredictionEngine`.
        """
...
class TargetPredictionEngine(nn.Module):
    ...
    def __init__(
        ...
        predictions. It includes normalization, optional positional embeddings, and a flexible sequence
        of decoding blocks.
        """
