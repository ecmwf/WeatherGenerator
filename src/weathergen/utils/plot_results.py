...
####################################################################################################
def plot_maps( cf, reportypes, targets_coords, values, masks_nan, 
                   title, cmap='bwr', root_err=False):
  ...
  for obs_idx in range(len(values)) :
    for ch in range( values[obs_idx].shape[-1]) :
      ...
      lats = np.clip( np.floor((90. - t_coords[...,idx_lats])).astype(np.int32), 0, 179).flatten()
      lons = np.clip( np.floor((180. + t_coords[...,idx_lons])).astype(np.int32), 0, 359).flatten()
      ...
      rtstr = '{}'.format(reportypes[obs_idx][1]).replace( ' ', '_').replace( '-', '_').replace(',','')
      fname = fig_path+'/{}/map_{}_ch{}_{}_{:05d}_{}.png'.format( rtstr, rtstr, chp,
                                                                  cf.run_id, epoch, title)
      ...
      im = ax.imshow( map, cmap=cmap, transform=cartopy.crs.PlateCarree(), vmin=vmin, vmax=vmax)
...
####################################################################################################
def plotExampleMap( reportype, fstep, step, fig_name, date, map_data, map_coords, ch, chp, err,
                    vmin=None, vmax=None, cmap='bwr', cmap_symmetric=False):
  ...
  title = f'{fig_name} : {reportype}::{chp} err={err}' # ({np.datetime_as_string( date, unit='m')})'
  fname = fig_path + '/example/{}/{}_{:05d}_{}_{}_{}_{:03d}_{:03d}.png'.format( reportype, cf.run_id, 
                                                                         epoch, reportype,
                                                                         chp, fig_name, fstep, step)
  print(fname)
  plotScatter( fname, title, lons, lats, data, cmap, vmin, vmax, cmap_symmetric)

##########################################################################################
def plotScatterMPL( filename, title, lats, lons, values, cmap='RdYlBu_r', vmin=None, vmax=None,
  ...
  im = plt.scatter( x=lons, y=lats, c=values,
                    cmap=cmap, s=0.25, alpha=1.0,
                    vmin=vmin, vmax=vmax, norm=divnorm,
...
##########################################################################################
def plotScatterDSH( filename, title, lats, lons, values, cmap='RdYlBu_r', vmin=None, vmax=None,
                    cmap_symmetric=False) :
...
####################################################################################################
def plotExampleMaps( cf, fstep, reportypes_active, cols_all, sources_all, targets_all,
                     preds_all, targets_coords_all, targets_idxs_all,
                     targets_lens_all,
                     bidx = 0, with_ens = False) :
  ...
  for k, (rt_idx, rt_name) in enumerate( reportypes_active) :
    ...
    if 'METEOSAT' in cf.streams[rt_idx]['name'] or 'Surface' in cf.streams[rt_idx]['name'] :
      ...
      for i,(date,idx) in enumerate( zip( dates_unique, idxs_unique)) : 
        ...
        mask = np.logical_and( np.logical_and( targets_coords[idx,1] == targets_coords[:,1], 
                                               targets_coords[idx,2] == targets_coords[:,2]), 
                                               targets_coords[idx,3] == targets_coords[:,3])

        errs_t = plotExampleMapsStream( name, fstep, i, cols_all[k], sources, sources_coords, 
                                    targets[mask], preds[mask], targets_coords[mask],
                                    date, vmin, vmax)
        errs += [ errs_t ]
      ...
      plotExampleMapsStream( name, fstep, 0, cols_all[k], sources, sources_coords, 
                             targets, preds, targets_coords, None)
    ...
    if len(errs)>0 :
      ...
      idxs = np.array([i for i in range(errs.shape[-1]) if np.nanmax(errs[:,i]) / 1000. <= np.nanmin(errs)])
      ...
      fig_name = fig_path + '/example/{}/{}_{}_error_lead_time.png'.format( name, cf.run_id, name)
      plt.savefig( fig_name)
      plt.close()

####################################################################################################
def plotExampleMapsStream( name, fstep, step, cols_all, sources, sources_coords, targets, preds, targets_coords, 
                                                 str_date = '', vmins = None, vmaxs = None) :
  ...
  for ch in range(len(cols_all)) :
    ...
    print( 'Processing {}, fstep={}, step={} :: ch={} ({}/{}) :: {} / {} / {}.'.format( name, 
            fstep, step, chp, ch, n_chs, sources.shape, targets.shape, preds.shape))
    ...
    if sources.shape[0] > 0 :
      plotExampleMap( name, fstep, step, 'source', str_date, sources, 
                      sources_coords, ch, chp, err, vmin, vmax, cmap)

    plotExampleMap( name, fstep, step, 'target', str_date, targets, 
                    targets_coords, ch, chp, err, vmin, vmax, cmap)
    ...
    plotExampleMap( name, fstep, step, 'prediction', str_date, preds.mean(1), 
                    targets_coords, ch, chp, err, vmin, vmax, cmap)

    if with_ens :
      for i_ens in range( preds.shape[1]) :
        plotExampleMap( name, fstep, step, 'prediction_ens{:02d}'.format(i_ens), str_date,
                        preds[:,i_ens], targets_coords, 
                        ch, chp, vmin, vmax, cmap)

    diff = targets - preds.mean(1)
    plotExampleMap( name, fstep, step, 'difference', str_date, diff, targets_coords, 
                    ch, chp,  err, None, None, cmap='bwr', cmap_symmetric=True)
...
def compute_mean_space_time( preds, targets, targets_coords_all, mask_nan, ch=0) :
  ...
  mask = np.logical_and( lons > 0., lons < 10.)
...
####################################################################################################
def plot_scanlines( streams, reportypes_active, targets_all, targets_coords_all, 
                    num_samples=512, cmap = 'bwr') :
  ...
  for stream in streams :
    ...
    for i in range( num_samples) :
      ...
      im = plt.scatter( x=lons, y=lats, c=values,
                      cmap=cmap, s=1, alpha=1.0, vmin=vmin, vmax=vmax,
                      transform=cartopy.crs.PlateCarree() )
...
if __name__ == '__main__' :
  ...
  parser.add_argument( '-fs', '--forecast_steps', nargs="+", default=[], help='forecast steps as list or range')
  ...
  if len(args.forecast_steps)==1 and '-' in args.forecast_steps[0] :
    fsteps = np.arange( int(args.forecast_steps[0].split('-')[0]), 
  ...
  for fstep in fsteps :
    ...
    if with_channel_errors :
      ...
      for obs_idx, (ii,reportype) in enumerate(reportypes_active) :
        ...
        fname += '_{}.csv'.format( reportypes_active[obs_idx][1].replace( ' ', '_').replace( '-', '_').replace(',',''))
        file_results = open( fname, 'w' if fstep==fsteps[0] else 'a')
        # TODO: add header

        cols = cols_all[obs_idx]
        cols = [c[9:] for c in cols if c[:9]=='obsvalue_'] if cf.streams[obs_idx]['type']=='obs' else cols
        ...
        for ch in range( p.shape[-1]) :
          ...
          if not np.all( ~mask_nan[...,ch]) :
            ...
            print( '{} : ch={} : \t {:4E} / {:4E} / {:4E} / {:4E} / {:4E}'.format( reportype, cols[ch], 
                                                  err_mse, err_mae, err_rmse, std_dev, err_mse_rel))
        ...
        if 'Surface' in reportype :
          ...
          print('{} : ch=10ff : \t {:4E} / {:4E} / {:4E}'.format( reportype, err_mse, err_mae, err_rmse))
    ...
    if with_avg_plots :
      ...
      # std-dev maps
      plot_maps( cf, reportypes_active, targets_coords_all, preds_std_all, masks_nan, 'ens-std')
      ...
      for i in range(len(targets_all)) :
        ...
        errs_chs = []
        for ch in range(p.shape[-1]) :
          errs_chs += [ np.expand_dims( np.square( p[...,ch] - t[...,ch]), axis=len(p.shape)-1) ]
        errs_all.append( np.concatenate( errs_chs, len(p.shape)-1) )
      plot_maps( cf, reportypes_active, targets_coords_all, errs_all, masks_nan, 'rmse', 'bwr', True)
    ...
    if len(example_samples) > 0 :
      ...
      if example_samples == [0] :
        ...
        for i in range(len(sources_all)) :
          sources_all[i] = np.split( sources_all[i], np.array(sources_lens_all[i]).cumsum(0)[:-1])
    ...
    for ex_idx in example_samples :
      ...
      try :
        ...
        plotExampleMaps( cf, fstep, reportypes_active, cols_all, sources_all,
                            targets_all, preds_all, targets_coords_all, targets_idxs_all,
                            targets_lens_all, bidx=ex_idx, with_ens=with_ens)
