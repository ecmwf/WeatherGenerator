...
class Trainer(TrainerBase):
    ...
    def train(self, mini_epoch):
        ...
        for bidx, batch in enumerate(dataset_iter):
            ...
            # EMA update
            if self.validate_with_ema:
                self.ema_model.update(self.cf.general.samples, self.batch_size_per_gpu * self.cf.world_size)
            ...
            self.cf.general.samples += self.batch_size_per_gpu * self.cf.world_size
    ...
    def _log(self, stage: Stage):
        ...
        if is_root():
            # plain logger
            if stage == VAL:
                self.train_logger.add_logs(stage, self.cf.general.samples, losses_all, stddev_all)
