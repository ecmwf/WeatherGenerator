...
class Trainer(TrainerBase):
    ...
    def _init_loss_spike_detection(self) -> None:
        self.loss_spike_cfg = self.cf.train_logging.get("loss_spike_detection", {})
        window_size = int(self.loss_spike_cfg.get("window_size", 50))
        self.loss_spike_history = deque(maxlen=window_size)
        self.loss_spike_file = None

        if not self.loss_spike_cfg.get("enabled", False):
            return

        self.loss_spike_file = config.get_path_run(self.cf) / self.loss_spike_cfg.get(
            "file_name", "loss_spikes.jsonl"
        )
    ...
    def _serialize_datetimes(self, datetimes) -> list[str]:
        ...
        max_unique = int(self.loss_spike_cfg.get("max_unique_times_per_step", 8))
    ...
    def _write_loss_spike_record(
        ...
        record = {
            ...
            "skip_batch": bool(self.loss_spike_cfg.get("skip_batch", True)),
            "source_samples": [
    ...
    def _maybe_log_loss_spike(self, loss_value: float, batch, mini_epoch: int, bidx: int) -> bool:
        ...
        min_history = int(self.loss_spike_cfg.get("min_history", 20))
        if len(self.loss_spike_history) >= min_history:
            ...
            is_large_enough = loss_value >= float(self.loss_spike_cfg.get("loss_threshold", 0.0))
            is_spike = ratio >= float(self.loss_spike_cfg.get("ratio_threshold", 5.0))
            if (is_finite and is_large_enough and is_spike) or not is_finite:
                ...
                should_skip = bool(self.loss_spike_cfg.get("skip_batch", True))
