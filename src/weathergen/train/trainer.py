...
class Trainer(TrainerBase):
    ...
    def save_model(self, mini_epoch: int, name=None):
        ...
        if is_root():
            ...
            # optim_state_dict is a list (one entry per optimizer, e.g. adamw + muon);
            # each optimizer's state is keyed by its own local param index, so map that
            # index back to the parameter name via the optimizer's param_groups.
            tensor_to_name = {id(p): n for n, p in self.model.named_parameters()}
            named_optim_state = {}
            for optimizer, opt_sd in zip(self.optimizers, optim_state_dict, strict=True):
                opt_params = [p for group in optimizer.param_groups for p in group["params"]]
                for local_idx, param_state in opt_sd["state"].items():
                    pname = tensor_to_name.get(id(opt_params[local_idx]))
                    if pname is not None:
                        named_optim_state[pname] = param_state
    ...
    def _load_optimizer_state(self, run_id: str, mini_epoch):
        ...
        # map each parameter to the optimizer that owns it (adamw or muon), so state
        # restores into the correct optimizer for the multi-optimizer (Muon) setup.
        param_to_optimizer = {}
        for optimizer in self.optimizers:
            for group in optimizer.param_groups:
                for p in group["params"]:
                    param_to_optimizer[id(p)] = optimizer

        loaded = 0
        for name, param in self.model.named_parameters():
            if name not in named_state:
                continue
            optimizer = param_to_optimizer.get(id(param))
