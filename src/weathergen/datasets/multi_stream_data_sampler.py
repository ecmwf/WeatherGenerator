...
class MultiStreamDataSampler(torch.utils.data.IterableDataset):
    def __init__(self, cf: Config, mode_cfg: dict, stage: Stage):
        ...
        self._init_healpix(cf)
        self._init_forecast_cfg(mode_cfg)
        self._init_time_window(mode_cfg)
        self._init_stream_datasets(cf)
        self._init_sampler_length(cf)
        ...
        # these fields change each mini-epoch → only defined in reset()
        self.rng = None
        self.perms = None
        self.perms_num_forecast_steps = None

    def _init_healpix(self, cf):
    ...
    def _init_forecast_cfg(self, mode_cfg):
        ...
        if self.forecast_policy != "random":
            ...
            self.fsm = self.list_num_forecast_steps[idx]
        else:
            self.fsm = self.list_num_forecast_steps.max()

        if self.fsm > 0:
            logger.info(f"forecast_steps : {self.fsm}")


    def _init_time_window(self, mode_cfg):
        ...
        forecast_len = (self.time_step * (self.fsm + 1))
        print(f"forecast_len={forecast_len}, time_window_len={self.len_timedelta}")
        if forecast_len > self.len_timedelta:
            logger.warning(
                    f"WARNING: forecast length {forecast_len} does not fit within time_window_len "
                    f"{self.len_timedelta} - overwriting default value."
                )
            self.len_timedelta = forecast_len

        available_samples = (
            (mode_cfg.end_date - mode_cfg.start_date - forecast_len)
            // self.step_timedelta
        )
        ...
        if not self.repeat_data and self.samples_per_mini_epoch:
            if self.samples_per_mini_epoch >= available_samples:
                ...
                logger.warning(
                    f"Using adjusted end date {new_end_date} instead of {mode_cfg.end_date} \
                        to avoid repeating data. Use repeat_data_in_mini_epoch to avoid this behavior."
                )
                end_date = new_end_date
            else:
                end_date = mode_cfg.end_date
        else:
            end_date = mode_cfg.end_date
        ...
        # compute base permutation range (static)
    ...
    def _init_sampler_length(self, cf):
        """Compute static sampler length (self.len)."""

        L = self.samples_per_mini_epoch

        # adjust for world size + batch size
        len_chunk = ((L // cf.world_size) // self.batch_size) * self.batch_size
