...
class MultiStreamDataSampler(torch.utils.data.IterableDataset):
    ...
    def __init__(
        ...
        with profiler.ProfilerSection("init data_laoder", profile=True):
    ...
    def advance(self):
        ...
        Advance epoch
        """
        with profiler.ProfilerSection("advance data_laoder", profile=True):
            self.epoch += 1
    ...
    def get_sources_size(self):
        with profiler.ProfilerSection("get_sources_size data_laoder", profile=True):
            return [
    ...
    def get_sources_num_channels(self):
        with profiler.ProfilerSection("get_sources_num_channels data_laoder", profile=True):
            return [ds[0].get_source_num_channels() for ds in self.streams_datasets]

    ###################################################
    def get_targets_num_channels(self):
        with profiler.ProfilerSection("get_targets_num_channels data_laoder", profile=True):
    ...
    def get_targets_coords_size(self):
        ...
        with profiler.ProfilerSection("get_targets_coords_size data_laoder", profile=True):
            return [
    ...
    ###################################################
    def reset(self):
        with profiler.ProfilerSection("reset data_laoder", profile=True):
            fsm = (
    ...
    def denormalize_source_channels(self, obs_id, data):
        with profiler.ProfilerSection("denormalize_source_channels data_laoder", profile=True):
    ...
    def denormalize_target_channels(self, obs_id, data):
        # TODO: with multiple ds per stream we need to distinguish these here
        with profiler.ProfilerSection("denormalize_target_channels data_laoder", profile=True):
    ...
    def __iter__(self):
        ...
        """
        with profiler.ProfilerSection("__iter__ data_laoder", profile=True):
            init_loggers()
                    ...
                    for stream_info, stream_ds in zip(self.streams, self.streams_datasets, strict=True):
                        ...
                        for _, ds in enumerate(stream_ds):
                            ...
                            if rdata.is_empty():
                                ...
                                with profiler.ProfilerSection("tokenizer_source data_laoder", profile=True):
                            ...
                            for fstep in range(
                                ...
                                if rdata.is_empty():
                                    ...
                                    with profiler.ProfilerSection("tokenizer_target  data_laoder", profile=True):
    ...
    def worker_workset(self):
        with profiler.ProfilerSection("worker_workset data_laoder", profile=True):
