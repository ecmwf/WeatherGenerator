...
def calculate_and_save_stats_file_by_file(
    ...
    """
    _logger.info(
        f"Starting global statistics calculation (file-by-file) for pattern: {input_glob_pattern}"
    )
    _logger.info(f"Variables to process: {variables}")
    if start_date_str:
        _logger.info(f"Overall start date filter: {start_date_str}")
    if end_date_str:
        _logger.info(f"Overall end date filter: {end_date_str}")
        ...
        try:
            ...
            with xr.open_dataset(file_path, chunks={"time": "auto"}) as ds_single_file:
                ...
                if ds_to_process.time.size == 0:
                    _logger.debug(
                        f"  No data in {Path(file_path).name} within the overall date range. Skipping."
                    )
                    continue
                ...
                for var_name in variables:
                    if var_name not in ds_to_process:
                        _logger.warning(
                            f"  Variable '{var_name}' not found in file {Path(file_path).name}. Skipping for this file."
                        )
                        continue

                    _logger.debug(
                        f"  Calculating partial stats for '{var_name}' in {Path(file_path).name}..."
                    )
                    ...
                    if valid_data.size > 0:
                        ...
                        _logger.debug(
                            f"    {var_name}: sum={current_sum:.2f}, count={current_count}, sum_sq={current_sum_sq:.2f}"
                        )
                    else:
                        _logger.debug(
                            f"    {var_name}: No valid data points in this file after filtering."
                        )
        ...
        if count_n > 0:
            ...
            if variance < 0:
                if np.isclose(variance, 0):
                    ...
                    _logger.warning(
                        f"Computed negative variance ({variance}) for {var_name}. Clamping to 0."
                    )
                    variance = 0.0
            std_dev = np.sqrt(variance)

            final_stats_to_save[var_name] = {
                "mean": float(mean),
                "std": float(std_dev),
                "count": int(count_n),
            }
            ...
            _logger.warning(
                f"No valid data points found for variable '{var_name}' across all files. Stats will be NaN."
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Calculate global statistics from RADKLIM NetCDF files (file-by-file)."
    )
    ...
    parser.add_argument(
        "--fail_on_file_error",
        action="store_true",  # Renamed for clarity
    ...
    if success:
        ...
        _logger.error(
            "Global statistics calculation encountered errors or failed for some variables/files."
        )
