...
class DataReaderAnemoi(DataReaderTimestep):
    ...
    def __init__(
        ...
        data_load_start :
            start of the range to open, when it must be wider than the sampler's
            time window. An operational analysis normally predates the window it
            initialises, so the operan reader opens a short pre-window lookback.
            Defaults to the time window's own start.
        data_load_end :
            end of the range to open. Defaults to the time window's own end.
        ...
        # is still usable -- which is routine for an analysis valid at the start
        # of the window it initialises. The end of the range remains exclusive.
        ...
        if "frequency" in stream_info:
            ...
            # range-restricted subset's own native frequency, which requires
            # >=2 dates inside [load_start, load_end) and raises otherwise. Skip
            # the (then redundant) kwarg when ds0 is already natively at that
            # frequency, so a sparse real-time store with one date in the range
            # doesn't crash on what would be a no-op anyway.
            try:
        ...
        if "statistics" in stream_info:
            # A rolling real-time store holds only a handful of analyses, so its
            # own statistics are meaningless. Normalise against the training
            # archive instead.
        ...
        if len(ds.dates) >= 2:
            ...
            # ds.frequency infers spacing from ds's own dates and needs >=2; a
            # sparse real-time store can have just one date inside the requested
            # range even though the full dataset has more.
    ...
    def _init_missing_data(
        ...
        An operational input stream that is momentarily without an analysis must
        keep its channel and normalization schema so the sampler can build a
        shape-correct spoof; every other stream keeps the existing behaviour of
