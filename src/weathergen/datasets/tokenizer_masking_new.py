...
class TokenizerMasking:
    ...
    def batchify_source(
        ...
        time_win = tuple(int(t.astype('datetime64[s]').astype('int64')) for t in time_win)
        ...
        if is_diagnostic or source.shape[1] == 0 or len(source) < 2 or cur_masking_rate == 1.0:
            ...
            return self._compiled_batchify_source(
                token_size,
                tokenize_spacetime,
                cur_masking_rate,
                coords,
    ...
    @torch.compile(fullgraph=True)
    def _compiled_batchify_source(
        self,
        token_size: int,
        tokenize_spacetime: bool,
        cur_masking_rate: float,
        coords: np.array,
        geoinfos: np.array,
        source: np.array,
        times: np.array,
        ...
        print("time_win inside _compiled_batchify_source :::",time_win)
        ...
        def cond_tokens_cells(c):
            length = torch.tensor(len(c))  # must be a tensor
            ...
            return torch.cond(length > 0, true_fn, false_fn)
        
        source_tokens_cells = [cond_tokens_cells(c) for c in source_tokens_cells]


        #source_tokens_lens = torch.tensor([len(s) for s in source_tokens_cells], dtype=torch.int32)

        def length_or_zero(s):
            is_empty = torch.tensor([len(s) == 0])
            return torch.cond(is_empty.item(), lambda: torch.tensor(0), lambda: torch.tensor(len(s)))

        source_tokens_lens = torch.tensor([length_or_zero(s) for s in source_tokens_cells], dtype=torch.int32)
        ...
        total_tokens = source_tokens_lens.sum()
        mask = torch.rand(total_tokens, device=source_tokens_lens.device) < cur_masking_rate
        ...
        rand_idx = torch.randint(low=0, high=len(mask), size=(1,), device=mask.device)

        update_true = torch.zeros_like(mask, dtype=torch.bool).scatter_(0, rand_idx, True)
        update_false = torch.zeros_like(mask, dtype=torch.bool).scatter_(0, rand_idx, True)
        ...
        split_indices = torch.cumsum(source_tokens_lens, dim=0)
        # Start indices for each chunk
        start_indices = torch.cat((torch.tensor([0], device=source_tokens_lens.device), split_indices[:-1]))
        # List to hold splits
        perm_sel = []
        for start, length in zip(start_indices, source_tokens_lens):
            chunk = mask[start : start + length]
            perm_sel.append(chunk)

        self.perm_sel = perm_sel
