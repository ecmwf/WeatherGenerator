...
Command:
uv run pytest  ./integration_tests/small1_chunk.py
"""
...
@pytest.mark.parametrize("test_run_id", ["test_small1_chunk_" + commit_hash])
def test_train(setup, test_run_id):
    ...
    main(
        [
            "train",
            f"--base-config={WEATHERGEN_HOME}/integration_tests/small1_chunk.yml",
            "--run-id",
            test_run_id,
        ]
    )
...
def infer(run_id):
    ...
    main(
        [
            ...
            run_id,
            "--config",
            f"{WEATHERGEN_HOME}/integration_tests/small1_chunk.yml",
        ]
    )
