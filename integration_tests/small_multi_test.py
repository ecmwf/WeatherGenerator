...
def test_train_multi(setup, test_run_id):
    logger.info(f"test_train with run_id {test_run_id} {WEATHERGEN_HOME}")

    train_with_args(
        f"--config={WEATHERGEN_HOME}/integration_tests/small_multi_test.yaml".split()
        + [
            "--run_id",
            test_run_id,
        ],
...
def infer(run_id):
    ...
    inference_from_args(
        ...
        + [
            ...
            run_id,
            "--config",
            f"{WEATHERGEN_HOME}/integration_tests/small_multi_test.yaml",
        ]
    )
...
def assert_val_loss_below_threshold(run_id):
    ...
    # This is meant to be a quick test, not a convergence test
    max_allowed = 1.25
    assert loss_metric < max_allowed, (
        f"'stream.ERA5.loss_mse.loss_avg' is {loss_metric}, expected to be below {max_allowed}"
    )
