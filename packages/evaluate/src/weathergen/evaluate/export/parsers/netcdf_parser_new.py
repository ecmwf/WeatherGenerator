...
class NetcdfParser(CfParser):
    ...
    def process_sample(
        ...
        **kwargs,
        ...
        for result in fstep_iterator_results:
            ...
            print("Xarray PROC-SAMPLE")
            print(result)
            print(result.lat.shape)
            print(result.lon.shape)
            result = self.reshape(result)
            print("Xarray PROC-SAMPLE -reshape")
            print(result)
            print(result.lat.shape)
            print(result.lon.shape)
            da_fs.append(result)
        ...
        if da_fs:
            if len(da_fs) > 1:
                print("LAT-LON NAN PROC. SAMPLE: ", np.isnan(get_grid_points(da_fs[1])).sum())
                print("LAT-LON SHAPE PROC. SAMPLE: ", get_grid_points(da_fs[1]).shape)
                assert np.array_equal(get_grid_points(da_fs[1]),
                                      get_grid_points(da_fs[0]),
                                      equal_nan=True), (
    ...
    def reshape(self, data: xr.DataArray) -> xr.Dataset:
        ...
        #### to make it working with hourly model:
        
        # Reshape ipoint and valid time. Now ipoint is a flattened (valid_time, spatial_point)
        n_vt = np.unique(data["valid_time"].values).size
        n_ipoint = data.sizes["ipoint"]
        assert n_ipoint % n_vt == 0, (f"ipoint ({n_ipoint}) is not divisible by n valid_time ({n_vt})")
