...
class Quaver_Parser(CF_Parser):
    ...
    def extract_var_info(self, var: str) -> tuple[str, str, str]:
        ...
        if not var_config:
            raise ValueError(f"Variable '{var} (using: {var_short})' not found in configuration mapping.")
    ...
    def get_output_filename(self, level_type: str) -> Path:
        ...
        """
        return Path(self.output_dir) / f"{self.data_type}_{level_type}_{self.run_id}_{self.expver}.{self.file_extension}"
