...
def get_data_worker(args: tuple) -> tuple[int, int, xr.DataArray]:
    ...
    print("**** NA ****")
    print(da_result.isnull().sum())
    print(np.isnan(coords_arr[:, 0]).sum())
    print(np.isnan(coords_arr[:, 1]).sum())

    da_result = da_result.dropna(dim = "ipoint")
    
    print("**** DROP NA ****")
    print(da_result.isnull().sum())
    print(da_result.shape)
    print(da_result["lat"].shape)
    print(da_result["lon"].shape)
