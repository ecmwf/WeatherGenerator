...
def regrid_gaussian_da(
    data: xr.DataArray, output_grid_type: str, degree: int, grid_shape: list
) -> xr.DataArray:
    """
    ...
    regrid_data = xr.DataArray(
        data=values, dims=dims, coords=coords, attrs=data.attrs, name=data.name
    )

    return regrid_data


def regrid_gaussian_ds(
    ds: xr.Dataset, output_grid_type: str, degree: float, indices: list
) -> xr.Dataset:
    """
    ...
    if output_grid_type == "regular_ll":
        ...
        raise ValueError(
            f'Unsupported grid_type: {output_grid_type}, supported types are ["regular_ll"]'
        )
    ...
    regrid_ds.attrs["history"] += (
        f" and regridded from O96 to {degree} degree regular lat lon using earthkit"
    )
    return regrid_ds
