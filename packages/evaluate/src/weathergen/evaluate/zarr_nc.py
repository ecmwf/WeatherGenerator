...
def output_filename(type, run_id, sample_idx, output_dir, output_format):
    ...
    out_fname = (
        Path(output_dir) / f"{type}_{run_id}_sample{sample_idx}.{file_extension}"
    )
    return out_fname
...
if __name__ == "__main__":
    ...
    valid_time = [
        [np.unique(da.valid_time).squeeze() for da in da_pred] for da_pred in da_preds
    ]
    ...
    for sample_idx, targ_list in targ_sample_all_steps.items():
        ...
        out_fname = output_filename(
            "targ", run_id, sample_idx, output_dir, output_format
        )
        _logger.info(f"Saving sample {sample_idx} to {out_fname}...")
        sample_all_steps = sample_all_steps.assign_coords(
            forecast_ref_time=forecast_ref_time[sample_idx]
