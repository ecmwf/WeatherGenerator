...
class Plotter(object):
    ...
    def histogram(self, target: xr.DataArray, preds: xr.DataArray, variables: list, tag = "", select: dict = {}):
        ...
        for var in variables:
            ...
            plt.hist(target.sel(select_var), bins=50, alpha=0.7, label='Target')
            plt.hist(preds.sel(select_var), bins=50, alpha=0.7, label='Prediction')
    ...
    def map(self, data: xr.DataArray, variables: list, tag: str = "", select: dict = {}):
        ...
        for var in variables:
            select_var = self.select | select | {"channel" : var}
            fig = plt.figure(figsize=self.fig_size, dpi=self.dpi_val)
            ax = fig.add_subplot(1, 1, 1) #fig.add_subplot(1, 1, 1, projection=ccrs.Robinson())
            # ax.coastlines()   
...
class LinePlots(object):
    ...
    def __init__(self, cfg: dict):
        ...
        self.out_dir = out_dir.joinpath(self.out_format)
        overwrite = OmegaConf.select(cfg, "overwrite") or False
    ...
    def plot(self, data: xr.DataArray | list, labels: str | list, tag: str = "", plot_dim="forecast_step"):
        ...
        for i, data in enumerate(data_list):
            averaged = data.mean(dim=[dim for dim in data.dims if dim != plot_dim])
            plt.plot(averaged[plot_dim], averaged.values, label = label_list[i], marker = "o", linestyle = "-")
