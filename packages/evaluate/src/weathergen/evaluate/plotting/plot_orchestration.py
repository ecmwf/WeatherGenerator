...
def plot_score_maps_per_stream(
    ...
    regions: list[str],
    metrics_dict: dict,
    global_plotting_opts: dict | None = None,
    plot_score_bias_animations: bool = False,
    ...
    global_plotting_opts : dict | None
        Top-level plotting options from the evaluation config.
    plot_score_bias_animations : bool
        Whether to animate the sample-aggregated bias score maps.
    ...
    if plot_score_bias_animations and any("bias" in region_metrics for region_metrics in metrics_dict.values()):
        plot_channels = channels if channels is not None else list(np.atleast_1d(da_preds[fsteps[0]].channel.values))
        ...
        _dispatch_score_map_animations(
            ...
            plot_channels,
            ensemble,
            plotter_cfg["image_format"],
            plotter_cfg["animation_format"],
            plotter_cfg["fps"],
            max_workers=max_wk,
        )
...
def _build_single_score_map_animation(
    map_output_dir: Path,
    run_id: str,
    stream: str,
    region: str,
    ...
    """Build one animation for a single bias score-map channel across forecast steps."""
    image_paths: list[str] = []
    for fstep in fsteps:
        tag = f"score_maps_bias_fstep_{fstep}" + (f"_ens_{ens}" if ens is not None else "")
        frame_path = map_output_dir / f"map_{run_id}_{tag}_{stream}_{region}_{var}.{image_format}"
        if frame_path.exists():
            image_paths.append(str(frame_path))

    if not image_paths:
        _logger.debug(f"No score-map images found for animation {region} {var} {ens}")
        return []

    out_tag = "score_maps_bias" + (f"_ens_{ens}" if ens is not None else "")
    out_path = map_output_dir / f"animation_{run_id}_{out_tag}_{stream}_{region}_{var}.{animation_format}"
    ...
    _logger.debug(f"Saved score-map animation to {out_path}")
    return image_paths
...
def _dispatch_score_map_animations(
    ...
    """Build bias score-map animations for all region/channel combinations."""
    duration_ms = int(1000 / fps) if fps and fps > 0 else 400
    ensemble_values = ensemble if ensemble is not None else [None]
    ...
    tasks = [
        ...
        for region in regions
        for ens in ensemble_values
        for var in variables
    ]

    calls = [
        delayed(_build_single_score_map_animation)(**t)
        for t in tqdm(tasks, desc=f"Creating score-map animations {stream} bias")
    ]
    results = dispatch_parallel(
        calls,
        n_workers=get_num_workers(max_workers=max_workers),
        backend="threading",
        desc="Score-map animations",
    )
    return [p for r in results if r for p in r]
