...
def infer_multi_stream(run_id):
    ...
    inference_from_args(
        ["-start", "2021-10-10", "-end", "2022-10-11", "--samples", "10", "--options", "forecast_offset = 0"]
        + [
            "--from_run_id",
            run_id,
            "--run_id",
            ...
            ",".join(streams),
            "--config",
            str("./config/evaluate/latent_space_eval_config.yaml")
        ]
    )
    return new_run_id
...
def print_losses(run_id, stage="val"):
    ...
    print(f"\n{stage.capitalize()} Losses for run_id: {run_id}")
    ...
    print(f"{stage_label} losses – " + ", ".join(f"{k}: {v:.4f}" for k, v in losses.items()))
    ...
    for stream_name in streams:
        ...
        for metric in metrics:
            ...
            for region in regions:
                ...
                print(mean_da.to_dataframe(name=f"{metric} {region} {stream_name}"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run_id", type=str, required=True, help="Run identifier for the model to evaluate")
    ...
    infer_run_id = run_id + "_inf" #infer_multi_stream(run_id)
