...
try:
    ...
    print("Could not import xskillscore and xhistogram. Thus, CRPS and rank histogram-calculations are not supported.")
...
def get_skill_score(score_model, score_ref, score_perf):
...
class Scores:
    ...
    def __init__(self, data_fcst: xr.DataArray, data_ref: xr.DataArray, avg_dims: str_or_list = "all", ens_dim: str = "ens"):
        ...
        self.det_metrics_dict = {"ets": self.calc_ets, "pss": self.calc_pss, "fbi": self.calc_fbi,
                                 ...
                                 "psnr": self.calc_psnr, "seeps": self.calc_seeps} 
    ...
    def calc_mae(self, **kwargs):
        ...
        if self.avg_dims is None:
            raise ValueError("Cannot calculate mean absolute error without average dimensions (avg_dims=None).")
    ...
    def calc_mse(self, **kwargs):
        ...
        if self.avg_dims is None:
            raise ValueError("Cannot calculate mean squared error without average dimensions (avg_dims=None).")
    ...
    def calc_rmse(self, **kwargs):
        ...
        """
        if self.avg_dims is None:
            raise ValueError("Cannot calculate root mean squared error without average dimensions (avg_dims=None).")
    ...
    def calc_seeps(self, seeps_weights: xr.DataArray, t1: xr.DataArray, t3: xr.DataArray, spatial_dims: List):
        ...
        if self.data_fcst.ndim == 3:
            assert len(spatial_dims) == 2, "Provide two spatial dimensions for three-dimensional data."
    ...
    def calc_spread(self, **kwargs):
        ...
        ens_std = self.data_fcst.std(dim = self.ens_dim)
    ...
    def calc_crps(self, method: str="ensemble", **kwargs):
        ...
        if method == "ensemble":
            ...
            func_kwargs = {"mu": self.data_fcst.mean(dim=self.ens_dim), 
                           "sig": self.data_fcst.std(dim=self.ens_dim), 
                           "dim": self.avg_dims, **kwargs}
