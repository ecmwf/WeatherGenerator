...
class DataReaderAnemoiOperan(DataReaderAnemoi):
    ...
    Stream config keys, in addition to those of the ``anemoi`` reader:

    ``analysis_availability``
        ``"nominal_time_mapping"`` (default) reproduces the historical archive
        behaviour: an analysis becomes available at the mapped hour given by
        ``nominal_time_mapping``. ``"dataset"`` is for a materialized real-time
        store, where only analyses that have actually been retrieved are present,
        so presence in the store *is* availability.
    ``analysis_lookback``
        How far before the window to open the dataset (default ``12h``). An
        analysis normally predates the window it initialises, so a sparse
        real-time store has to be opened with a pre-window lookback.
    ``max_analysis_age``
        Refuse an analysis older than this relative to the window end, so a
        missed cycle spoofs the input rather than silently initialising the
        forecast from stale state. Unset means no limit.
    ``analysis_available_until``
        Pin the as-of boundary, so re-running an old cycle against a store that
        has since grown cannot leak a later analysis into it. Unset means the
        window end is the only bound.

    Combine with the ``anemoi`` reader's ``statistics`` key to normalise against
    the training archive rather than the handful of analyses actually held.
    """
